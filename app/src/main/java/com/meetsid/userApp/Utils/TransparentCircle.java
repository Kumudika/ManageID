package com.meetsid.userApp.Utils;

import android.content.Context;
import android.content.res.TypedArray;
import android.graphics.Bitmap;
import android.graphics.Canvas;
import android.graphics.Paint;
import android.graphics.PorterDuff;
import android.graphics.PorterDuffXfermode;
import android.util.AttributeSet;
import android.view.View;

import com.meetsid.userApp.R;

public class TransparentCircle extends View {
    Bitmap bm;
    Canvas cv;
    Paint eraser;
    int color;

    public TransparentCircle(Context context) {
        super(context);
        Init();
    }

    public TransparentCircle(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.color_view);
        try {
            color = a.getColor(R.styleable.color_view_circle_color, getResources().getColor(R.color.meetsid_grey3));
//            setBackgroundColor(color);
        } finally {
            a.recycle();
        }
        Init();
    }

    public TransparentCircle(Context context, AttributeSet attrs,
                             int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        Init();
    }

    private void Init() {

        eraser = new Paint();
        eraser.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.CLEAR));
        eraser.setAntiAlias(true);
    }

    @Override
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {

        if (w != oldw || h != oldh) {
            bm = Bitmap.createBitmap(w, h, Bitmap.Config.ARGB_8888);
            cv = new Canvas(bm);
        }
        super.onSizeChanged(w, h, oldw, oldh);
    }

    @Override
    protected void onDraw(Canvas canvas) {
        int w = getWidth();
        int h = getHeight();
        int radius = w / 3;

        bm.eraseColor(color);
        cv.drawColor(getResources().getColor(R.color.meetsid_white));
        cv.drawCircle(w / 2, h / 2, radius, eraser);
        Paint paint = new Paint();
        paint.setStyle(Paint.Style.STROKE);
        paint.setStrokeWidth(20);
        paint.setColor(getResources().getColor(R.color.meetsid_green));
        canvas.drawCircle(w / 2, h / 2, radius, paint);
        canvas.drawBitmap(bm, 0, 0, null);
        super.onDraw(canvas);
    }
}
